...
//doesn't reset actual encoder readings, only display value
void W_Execute::on_encoderReset_clicked() {
    int slaveIndex = ui->comboBox_slave->currentIndex();
    currentEncVals[slaveIndex] = *((*deviceList)[slaveIndex].exList[slaveIndex]->enc_ang);
}
...
void W_Execute::display(struct execute_s *ex)
{
    ...
    ui->disp_encDisplay->setText(QString::number(*(ex->enc_ang)-currentEncVals[slaveIndex]));
