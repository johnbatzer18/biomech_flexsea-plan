...
void MainWindow::initializeCreateWindowFctPtr(void)
{
    ...
    mdiCreateWinPtr[STEP_WINDOWS_ID] = &MainWindow::createViewStep;
    }
...
//Creates a new Step Response window
void MainWindow::createViewStep(void)
{
    int objectCount = W_StepResponse::howManyInstance();
    ...
    if(objectCount < (STEP_RESPONSE_WINDOWS_MAX))
    {
        ...
        sendWindowCreatedMsg(W_StepResponse::getDescription(), objectCount,
                             W_StepResponse::getMaxWindow() - 1);

        //Link to MainWindow for the close signal:
        connect(myViewStep[objectCount], &W_StepResponse::windowClosed, \
                this,						&MainWindow::closeViewStep);

        //Link to SlaveComm to send commands:
        connect(myViewStep[objectCount], &W_StepResponse::writeCommand, \
                comManager,					&ComManager::enqueueCommand);
    }
    else
    {
        sendWindowCreatedFailedMsg(W_StepResponse::getDescription(),
                                   W_StepResponse::getMaxWindow());
    }
}

void MainWindow::closeViewStep(void)
{
